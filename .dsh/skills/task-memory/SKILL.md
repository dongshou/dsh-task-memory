...
| 项目层 | 仓库 Wiki（页面结构按项目实际定义，见下方"Wiki 页面定义要求"；Wiki 未激活时临时用置顶 issue #1） | 任务关闭时：把"别的任务还需要"的结论蒸馏进 Wiki |
...
页面目录**不预设**，按项目实际情况定义，只守四条要求：

1. **用项目自己的语言命名**（如"环境与部署""已知问题"），不用知识类型术语（如"决策记录""踩坑集"）；
2. **按项目天然存在的主题分页**：代码结构、运行部署、团队约定、演进历史、已知问题等，缺什么补什么；
3. **首页 = 项目简介 + 页面索引**；
4. **有真实内容才建页**，不预先建空页。

示例（协议主仓库 `dongshou/dsh-workhere` 的 Wiki）：Home（简介+索引）/ 架构 / 环境与部署 / 变更与决策 / 已知问题——仅作参考，不是模板。
